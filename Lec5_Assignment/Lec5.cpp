...
public:
    ...
    void addChild(int parent, int child) 
    {
        ...
        if (parentNode == NULL) 
        { // Nếu cây rỗng, đặt node cha làm node gốc
            root = makeNode(parent);
            parentNode = root;
        }

        if (parentNode->pLeft == NULL) 
        { // Nếu node con trái trống, thêm vào
            parentNode->pLeft = makeNode(child);
        } else if (parentNode->pRight == NULL) 
        { // Nếu node con phải trống, thêm vào
    ...
    void printPre(node* root) 
    {
        ...
        printPre(root->pLeft); // Đệ quy duyệt cây con trái
        printPre(root->pRight); // Đệ quy duyệt cây con phải
    }
    ...
    {
        if (root == nullptr) return; // Nếu cây rỗng, kết thúc
        printPost(root->pLeft); // Đệ quy duyệt cây con trái
        printPost(root->pRight); // Đệ quy duyệt cây con phải
    ...
    void printIn(node* root) 
    {
        ...
        { // Nếu không phải cây nhị phân, thông báo lỗi
            cout << "NOT BINARY TREE" << endl;
            return;
        }
        printIn(root->pLeft); // Đệ quy duyệt cây con trái
        cout << root->data << " "; // In giá trị node
        printIn(root->pRight); // Đệ quy duyệt cây con phải
    }
};
...
int main() {
    ...
    tree.printPre(tree.root); // In ra PreOrder
    cout << endl;
    tree.printPost(tree.root); // In ra PostOrder
    cout << endl;
    tree.printIn(tree.root); // In ra InOrder
    return 0;
}